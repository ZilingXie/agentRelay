...
Task-scoped file attachments shipped 2026-09-01 through Server [PR #90](https://github.com/ZilingXie/agentRelay/pull/90) and signed bundle revision 10: participant-gated `POST/GET /agentrelay/tasks/{task_id}/files[/file_id]` with streamed sha256 uploads capped at `AGENTRELAY_MAX_FILE_BYTES` (default 64 MiB, nginx `client_max_body_size 65m`), a dedicated `agentrelay-files.sqlite3` plus `data/blobs/` store, reply-only file parts with uploader==actor enforcement, and orphan (24 h) plus terminal-task retention (72 h) GC on the coordinator tick. Client 0.5.0 ([agent-relay-mcp #91](https://github.com/ZilingXie/agent-relay-mcp/pull/91)) binds approval to file content and uploads only after approval. Initial create/follow-up messages cannot carry file parts; the v0.5 lane is unaffected.

File-transfer hardening for signed bundle revision 11 is implemented on `fix/file-transfer-hardening`: upload body allowance is isolated to the file route while JSON remains capped at 1 MiB; deduplication includes uploader plus immutable name/MIME; recipients cannot list/download unreferenced uploads; Messages are capped at 8 files and 64 MiB aggregate; old file databases migrate under an SQLite write lock; stale crash blobs are reclaimed; and GC is throttled to an hourly configurable interval. The paired MCP client 0.5.1 uses streaming hash/upload/download, sequential bounded uploads, collision-free atomic download paths, exact upload-response metadata checks, and an explicit bounded attachment policy for Project Hermes.
...
Status: active in production as of 2026-07-28. The database migration, all
production Listener upgrades, offline recovery E2E, and post-cutover fixes are
complete.
...
The base implementation merged in order as Server PR #74, Client PR #67, then
Hermes PR #7. Activation followed through Server PR #76 and Hermes PR #8, with
the production database rehearsed from an online SQLite backup before the
maintenance cutover. Server PR #77 raised the v0.6 bundle revision to 6 so
upgraded Clients would not reject it as a downgrade. Server PR #78 made v0.6
push failure and ACK lease expiry park immediately while preserving v0.5 retry
behavior. Client PR #69 stamps recovered Events with the negotiated protocol
before durable inbox intake so v0.6 recovery uses the Message ACK route.

Production now publishes `agent-collab-v0.6`, `write_mode=v06`, and bundle
revision 6. Zac, Vivi, and Project Hermes all report fresh v0.6 readiness. Task
`task_cb366d360b2d4174a6cddc21de31a0c3` verified offline creation as
`open`/`pending`/`parked` with `waiting_listener`, Vivi recovery to delivered,
the Vivi reply and Zac ACK, and requester completion at Task version 5. No real
WeCom test notification was sent during activation.
...
- Observe v0.6 parked backlog, recovery latency, stale epoch rejection, terminal
  notice recovery, and compatibility traffic before narrowing the v0.5
  acceptance window.
